...
class PHALP(nn.Module):
    ...
    def track(self):
        ...
        self.default_setup()
        os.makedirs(self.cfg.video.output_dir + '/' + self.cfg.video_seq, exist_ok=True)
        ...
        try: 
            ...
            for t_, frame_name in progress_bar(enumerate(list_of_frames), description="Tracking : " + self.cfg.video_seq, total=len(list_of_frames), disable=False):
                ...
                for key in custom_keys: final_visuals_dic[frame_name][key] = []                
                ...
                for tracks_ in self.tracker.tracks:
                    ...
                    for key in custom_keys: final_visuals_dic[frame_name][key].append(track_data_hist[key])
                    ...
                    if(tracks_.time_since_update==0):
                        ...
                        if(tracks_.hits==self.cfg.phalp.n_init):
                            for pt in range(self.cfg.phalp.n_init-1):
                                ...
                                for key in custom_keys: final_visuals_dic[frame_name_][key].append(track_data_hist_[key])                                
                ...
                if(self.cfg.render.enable and t_>=self.cfg.phalp.n_init):                    
                    ...
                    for t__ in range(t_, t_+d_):
                        ...
                        # Render and save joint positions
                        dic = final_visuals_dic[frame_key]
                        for tid in range(len(dic['tid'])):
                            
                            # Determine 2d and 3d joint positions
                            box_center = dic['center'][tid]
                            ...
                            for idx in range(len(pred_keypoints_2d)):
                                pos = list(map(int, list(pred_keypoints_2d[idx])))
                                clamp = lambda x, y: (min(max(x, 0), rendered_.shape[0]-1), min(max(y, 0), rendered_.shape[1]-1))
                                z = 2
                                for i_ in range(-z, z+1):
                                    for j_ in range(-z, z+1):
                                        ii_, jj_ = clamp(pos[1]+i_, pos[0]+j_)
                                        rendered_[ii_][jj_] = [0, 0, 0]
                                                        
                            # Save 2d and 3d keypoints
                            if not frame_counter:
                                frame_counter = 1
                                frame_counter_offset = t__
                                
                            self.keypoints_2d.append(pred_keypoints_2d)
                            self.keypoints_3d.append(pred_keypoints_3d)
            ...
            # save custom data
            with open(self.cfg.video.output_dir + f"/{self.cfg.video_seq}/{self.cfg.video_seq}_metadata.json", "w") as metadata_file:
                json.dump(self.metadata, metadata_file)
            np.save(self.cfg.video.output_dir + f"/{self.cfg.video_seq}/{self.cfg.video_seq}_keypoints_3d", np.array(self.keypoints_3d))
            np.save(self.cfg.video.output_dir + f"/{self.cfg.video_seq}/{self.cfg.video_seq}_keypoints_2d", np.array(self.keypoints_2d))
    ...
    def cached_download_from_drive(self, additional_urls=None):
        ...
        if not os.path.exists(smpl_path):
            ...
            os.makedirs(os.path.join(CACHE_DIR, "phalp/3D/models/"), exist_ok=True)
            ...
            os.system('cp /clifford-data/home/pingpong-daniel/ppr/HumanPoseTracker/data/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl  ' + smpl_path)
